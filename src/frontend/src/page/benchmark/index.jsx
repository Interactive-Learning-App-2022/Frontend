...
class Benchmark extends React.Component {
  ...
  loadClassList() {
    ...
    _class.getAllPList(UserInfo).then(
      (res) => {
        ...
        for (var i = 0; i < res.data.length; i++) {
          if (!this.contains(res.data[i].proficiency_id.id, plist)) {
            ...
            pscores[res.data[i].proficiency_id.id] = {
              name: res.data[i].proficiency_id.name,
              p: 0,
              ap: 0,
              np: 0,
            };
        ...
        for (var i = 0; i < res.data.length; i++) {
          ...
          else pscores[res.data[i].proficiency_id.id]["np"] += 1;
        }
  ...
  onSubmit() {
    ...
    console.log(data);
    _class.assignTask(loginInfo, JSON.stringify(data)).then(
      (res) => {
        ...
        return (
          <tr
            ...
            onClick={(e) => {
              this.goTo(
                e,
                `/classroom/${this.state.classID}/${target.proficiency_id.id}`
              );
            }}
            id={index}
          >
            ...
            <td>
              ...
              <button
                ...
                data-toggle="modal"
                data-target={"#moreDetailsModal" + target.id}
              >
                More Details
              </button>
              <div
                class="modal fade"
                id={"moreDetailsModal" + target.id}
                tabindex="1"
                role="dialog"
                data-id={"span" + index}
                ...
                <div class="modal-dialog" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title" id="moreDetailsModalLabel">
                        {"Target " + target.proficiency_id.id}
                      </h5>
                      <button
                        type="button"
                        class="close"
                    ...
                    <div class="modal-body">{target.details}</div>
                  </div>
                </div>
              </div>
            </td>
            <td
              ...
              style={
                this.state.scores[target.proficiency_id.id].p != 0
                  ? { backgroundColor: "#01CF85" }
                  : { backgroundColor: "#01CF85" }
              }
            >
              {this.state.scores[target.proficiency_id.id].p}
            </td>
            <td
              className="text-center"
              style={
                this.state.scores[target.proficiency_id.id].ap != 0
                  ? { backgroundColor: "#FFD800" }
                  : { backgroundColor: "#FFD800" }
              }
            >
              {this.state.scores[target.proficiency_id.id].ap}
            </td>
            <td
              className="text-center"
              style={
                this.state.scores[target.proficiency_id.id].np != 0
                  ? { backgroundColor: "#FE4C4C" }
                  : { backgroundColor: "#FE4C4C" }
              }
            >
              {this.state.scores[target.proficiency_id.id].np}
            </td>
    ...
    return (
      <div id="page-wrapper">
        ...
        <button
          ...
          style={{ marginTop: "50px", backgroundColor: "#02D0FF" }}
        >
